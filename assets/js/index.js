...
function default_1(chanson) {
    ...
    section.innerHTML = "<h1 class=\"small-caps\">" + title + "</h1>\n    <p class='f6 i'>" + (author ? "<span class=\"author\">" + author + ",</span>" : '') + "\n    <em class=\"date\">derni\xE8re modification le " + moment_1["default"](date).format('LLL') + "</em>. <a href=\"" + url + "\">permalien</a></p>";
...
ready(function () {
    ...
    forEach_1["default"](chansons, function (chanson) {
        chanson.content = chanson.content.replace(/\\colonneSuivante/g, '<div class="colonne-suivante"></div>').replace(/\\sauterLigne{(\d+)}/g, function (ignore, nb) {
            return '<br/>'.repeat(nb - 1);
        }).replace(/\\choeur{([^}]+)}/g, '[$1]').replace(/\\vspace{([^}]+)}<br \/>/g, '<br style="display: block; margin: $1; line-height: 0px; content: \' \';" />');
        console.log(chanson.title);
        main.appendChild(chansonTemplate_1["default"](chanson));
    });
});
